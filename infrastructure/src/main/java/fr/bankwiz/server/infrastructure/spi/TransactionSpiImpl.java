...
public class TransactionSpiImpl implements TransactionSpi {
    ...
    @Override
    public List<TransactionDomain> findByBankAccount(BankAccountDomain bankAccount) {
    ...
    @Override
    public Optional<TransactionDomain> findById(UUID uuid) {
        final Optional<TransactionEntity> optional = this.transactionEntityRepository.findById(uuid);
        if (optional.isPresent()) {
            final TransactionDomain transaction = TransactionTransformer.fromTransactionEntity(optional.get());
